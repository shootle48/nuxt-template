<template>
  <div class="container mx-auto px-4 py-8">
      ...
      <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8">
          <button @click="activeTab = 'all'" 
            ...
            All ({{ myBorrowingRequests.length }})
          </button>
          ...
          <div class="flex-1">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm text-gray-600">
              <div>
                ...
                <p v-if="request.quantity > 1"><strong>Quantity:</strong> {{ request.quantity }}</p>
              </div>
              <div>
                <p><strong>Requested:</strong> {{ formatDateTime(request.created_at) }}</p>
                <p><strong>Duration:</strong> {{ request.duration }} days</p>
                <p v-if="request.approved_at"><strong>Approved:</strong> {{ formatDateTime(request.approved_at) }}</p>
              </div>
            </div>
            ...
            <div v-if="request.admin_notes" class="mt-3 p-3 bg-blue-50 rounded">
              <p class="text-sm"><strong>Admin Notes:</strong> {{ request.admin_notes }}</p>
            </div>
          </div>
          
          <!-- Status Badge -->
          <div class="ml-6">
            <div v-if="request.status === 'approved'" class="text-center">
              <div class="text-green-600 font-semibold">✓ Approved</div>
              <div class="text-xs text-gray-500 mt-1">Ready for pickup</div>
            </div>
            <div v-else-if="request.status === 'pending'" class="text-center">
              <div class="text-yellow-600 font-semibold">⏳ Pending</div>
              <div class="text-xs text-gray-500 mt-1">Waiting for approval</div>
            </div>
            <div v-else-if="request.status === 'rejected'" class="text-center">
              <div class="text-red-600 font-semibold">✗ Rejected</div>
              <div class="text-xs text-gray-500 mt-1">Request denied</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="filteredRequests.length === 0" class="text-center py-12">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">No borrowing requests</h3>
      <p class="mt-1 text-sm text-gray-500">You haven't made any borrowing requests yet.</p>
      <div class="mt-6">
...
const pendingRequests = computed(() => 
  myBorrowingRequests.value.filter(req => req.status === 'pending')
)
const approvedRequests = computed(() => 
  myBorrowingRequests.value.filter(req => req.status === 'approved')
)
const rejectedRequests = computed(() => 
  myBorrowingRequests.value.filter(req => req.status === 'rejected')
)
...
const filteredRequests = computed(() => {
  switch (activeTab.value) {
    ...
    default: return myBorrowingRequests.value
  }
})
